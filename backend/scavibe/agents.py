...
class OpenAISettings:
    """No default model is used because model selection is an owner decision."""

    model: str

    @classmethod
    def from_environment(cls) -> "OpenAISettings":
        model = os.environ.get("SCAVIBE_OPENAI_MODEL", "").strip()
        if not model:
            raise RuntimeError("SCAVIBE_OPENAI_MODEL is required; no model is selected by default")
...
class OpenAIResponsesGateway:
    """Thin adapter around the official asynchronous Responses API client."""

    def __init__(self, settings: OpenAISettings) -> None:
        self._settings = settings
        # The SDK is imported only when a real remote run is requested. This
        # keeps deterministic contract tests independent from network packages.
        try:
    ...
    async def generate(self, *, system_prompt: str, input_json: str) -> str:
        ...
        if not response.output_text:
            raise AgentProtocolError("OpenAI returned no output_text")
